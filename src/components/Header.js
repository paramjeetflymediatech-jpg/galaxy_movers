...
export default function Header() {
  ...
  return (
    ...
    >
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <Link
                ...
                className={`font-semibold text-sm transition-colors duration-200 hover:text-red-600 ${pathname === link.href
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-screen opacity-100 py-4 border-t border-gray-100 bg-white' : 'max-h-0 opacity-0 pointer-events-none'
          }`}
        id="mobile-menu"
      >
        <div className="px-4 space-y-3">
          {navLinks.map((link) => (
            <Link
              ...
              href={link.href}
              onClick={(e) => handleLinkClick(e, link.href)}
              className={`block px-3 py-2.5 rounded-lg text-base font-semibold transition-colors ${pathname === link.href
